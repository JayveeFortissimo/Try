    ...
    <form onSubmit={submitBoards} className="flex flex-col gap-3">
      <div>
        ...
        <Input
          ...
          placeholder="Title"
          onChange={(e) => getForBoards("name", e.target.value)}
        />
      </div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => getForBoards("subtitle", e.target.value)}
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          onChange={(e) => getForBoards("description", e.target.value)}
      ...
      <div>
        ...
        <select
          onChange={(e) => getForBoards("color", e.target.value)}
          disabled={sendLoading}
          className="w-full border p-3 rounded cursor-pointer"
        >
          <option value="#EADFCF">Oat Beige</option>
          <option value="#EFE7D8">Linen</option>
          <option value="#F6F1EA">Cloud Cream</option>
          <option value="#C9B8A6">Mushroom</option>
          <option value="#FFFFFF">Pure White</option>
        </select>
      </div>
      <Button disabled={sendLoading} type="submit">
        {sendLoading ? <SpinnerCircle2 /> : "Create Board"}
      </Button>
    </form>
  );
};
